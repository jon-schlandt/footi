...
struct CoreDataContext {
    ...
    func loadLeagues() async {
        var leaguesNeeded = [Int]()
        
        let existingLeagues = try? moc.fetch(LeagueEntity.fetchRequest())
        let existingLeagueIds = existingLeagues?.map { Int($0.id) }
        guard let existingLeagueIds = existingLeagueIds else {
            return
        }
        
        let availableLeagues = userDefaultsContext.getLeagues()
        let availableLeagueIds = availableLeagues?.keys.map { availableLeagues?[$0]?["id"] as! Int }
        guard let availableLeagueIds = availableLeagueIds else {
            return
        }
        
        availableLeagueIds.forEach { id in
            if !existingLeagueIds.contains(id) {
                leaguesNeeded.append(id)
            }
        }
        
        for id in leaguesNeeded {
            await loadLeague(for: id)
        }
    }
